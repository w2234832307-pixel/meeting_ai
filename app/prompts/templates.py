...
🚨🚨🚨 **最后检查清单 - 生成前必须确认**：
1. ✅ 所有人名是否已替换为【名称标准化映射表】中的标准全名？（不允许出现昵称、简称）
2. ✅ 所有项目名是否已替换为【名称标准化映射表】中的标准全称？（不允许出现口语化简称）
3. ✅ 所有公司名是否已替换为【名称标准化映射表】中的正式名称？（不允许出现俗称）
4. ✅ 是否使用了实际内容而非模板占位符？
5. ✅ 是否所有信息都来自转录文本？

⚠️ 再次强调：名称标准化是最高优先级！必须严格执行映射表中的所有替换规则！
""",
        "variables": {},
        "dynamic_sections": {
            "history_section": """
